...
(function () {
    ...
    function injectStyles() {
        ...
        style.textContent = [
            ...
            '.bdl-modal { position: relative; width: 100%; max-width: 640px; max-height: 90vh; overflow-y: auto; border-radius: 16px; background: #14151a; border: 1px solid rgba(255,255,255,0.08); padding: 2rem 1.75rem 1.75rem; box-shadow: 0 24px 60px rgba(0,0,0,0.4); color: rgba(255,255,255,0.92); }',
            'body.lightmode .bdl-modal { background: #fff; border-color: rgba(0,0,0,0.08); color: #111; }',
            ...
            'body.lightmode .bdl-close { background: rgba(0,0,0,0.06); color: #111; }',
            '.bdl-title { font-size: 1.15rem; font-weight: 800; margin: 0 0 0.3rem; color: #fff; }',
            'body.lightmode .bdl-title { color: #111; }',
            '.bdl-sub { font-size: 0.82rem; color: rgba(255,255,255,0.6); margin: 0 0 1.5rem; line-height: 1.5; }',
            'body.lightmode .bdl-sub { color: rgba(0,0,0,0.6); }',
            '.bdl-fields { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem 1.25rem; margin-bottom: 0.9rem; }',
            '.bdl-field { display: flex; flex-direction: column; gap: 0.35rem; }',
            '.bdl-field--full { grid-column: 1 / -1; }',
            '.bdl-field label { font-size: 0.78rem; font-weight: 600; color: rgba(255,255,255,0.75); }',
            'body.lightmode .bdl-field label { color: rgba(0,0,0,0.7); }',
            '.bdl-field input, .bdl-field textarea { width: 100%; padding: 0.65rem 0.85rem; border-radius: 8px; border: 1px solid rgba(255,255,255,0.12); background: rgba(255,255,255,0.04); color: #fff; font-size: 0.87rem; outline: none; font-family: inherit; transition: border-color 0.2s ease; }',
            'body.lightmode .bdl-field input, body.lightmode .bdl-field textarea { border-color: rgba(0,0,0,0.12); background: rgba(0,0,0,0.02); color: #111; }',
            ...
            'body.lightmode .bdl-submit { color: #fff; }',
            '.bdl-submit:hover { background: #e07010; }',
            '.bdl-submit:disabled { opacity: 0.6; cursor: not-allowed; }',
            '@media (max-width: 600px) { .bdl-fields { grid-template-columns: 1fr; } }'
        ].join('\n');
            ...
            '<div class="bdl-modal" role="dialog" aria-modal="true" aria-labelledby="bdlTitle">' +
                ...
                '<form id="bdlForm" novalidate>' +
                    '<div class="bdl-fields">' +
                        ...
                        '<div class="bdl-field bdl-field--full"><label for="bdlInfo">Additional Information</label><textarea id="bdlInfo" name="info"></textarea></div>' +
                    '</div>' +
